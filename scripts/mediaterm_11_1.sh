...
while getopts "A:B:bghHK:lL:noqQstuvw" opt; do
    case $opt in
        ...
        v)  if [ ! -f $dir/filmliste ]; then
            ...
            echo "MediaTerm 11.1, 2021-10-21"
...
function hits {
    ...
    # Ausgabe der Ergebnisliste bei nichtleerer Treffermenge
        ...
        if [ ! -z $lopt ]; then
            echo "$out" | \
                awk -F "\",\"" '{printf "("NR") " "\033[0;32m"$4"\033[0m"" ("$1")  [n/"} {if($4 == "3Sat Livestream" || $4 == "SR Livestream"){print "-]"} else{print "h]"}}; {print "\033[0;34m"$10"\033[0m","\n"}' | \
    ...
    if [ "$input" = k ]; then
        ...
        if [[ $filmnr -gt $trefferzahl || $filmnr -eq 0 ]]; then
            ...
            if [[ ! -z $lopt && ! $input =~ ^(i|w|b)[0-9]+$ ]]; then
                aud=0
                if wget -q -O - $filmurl | grep -E "wdrlz|TYPE=AUDIO" > /dev/null;then
                    aud=1
                fi
