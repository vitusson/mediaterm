...
#################################################################
#
#  Copyright (c) 2017-2024 Martin O'Connor (maroc@mailbox.org)
#
...
#### OPTIONEN
while getopts "A:B:beghHkK:lL:nostuvw" opt; do
    case $opt in
        b)  bopt=1
            ;;
        A)  Aopt=1
        ...
        v)  if [ ! -f $dir/filmliste ]; then
                ...
                flstand=$(head -n +1 $dir/filmliste | cut -d"\"" -f6 | tr -d '\n'; echo " ($(($(wc -l < $dir/filmliste) - 1)) Filme)")
            fi
            echo "MediaTerm 14.0, 2024-02-08"
...
${fett}AUFRUF:${normal}
      mediaterm [Optionen] [Suchbegriff(e)]
Suchbegriffe können ganze Wörter, aber auch Teilwörter sein.
...
${fett}SUCHOPTIONEN (auch auf der internen Eingabezeile nutzbar):${normal}
   Mit Ausnahme der Option -l muss auf die folgenden Optionen mindestens ein Suchbegriff folgen.
      ...
      ${fett}-e${normal}   Entfernt Dubletten mit identischen Film-URLS aus den Suchergebnissen.
...
${fett}SUCH-OPERATOREN:${normal}
       ${fett}+${normal}   Ein "+" unmittelbar vor einem Suchbegriff bewirkt, dass dieser als Einzelwort gesucht wird und NICHT als Zeichenfolge auch innerhalb von Wörtern.
       ${fett}~${normal}   Eine Tilde (~) unmittelbar vor einem Suchbegriff schließt diesen für die Suche gezielt aus. Dieser Operator kann nicht mit dem ersten Suchbegriff verwendet werden.
       ...
       ${fett}_${normal}   Ein Unterstrich (_) unmittelbar vor einem Suchbegriff bewirkt, dass dieser nur im Filmthema gesucht wird.
...
${fett}ANWENDUNGSBEISPIELE:${normal}
   ...
   ${unterstr}mediaterm -now alpen klimawandel${normal}
      ... liefert die gleiche Trefferliste in roher Form, d.h. ohne Kommandoeingabe (-n), ohne Farbe (-o) und ohne worterhaltende Zeilenumbrüche (-w). Dies ist sinnvoll, wenn die Liste weiterverarbeitet oder in eine Datei umgeleitet werden soll.
   ...
   ${unterstr}mediaterm schlaf _"science talk"${normal}
      ... sucht nach Treffern mit dem Thema "Science Talk"", die den Suchbegriff "schlaf" enthalten.
...
while true; do
    ...
    #Anzeige von 5 bzw. 10 Treffern ab Zeile
        ...
        min=$(( trefferzahl < im ? trefferzahl : im )) 
        a=$(( a > 0 ? a : trefferzahl-min+1 ))
        let x1=(a-1)*tz+1
        let x2=(im*tz)+x1-1
        let a2=a+im-1
        pl=im
        ...
        if [ "$a" = "" ]; then
            echo "Blättern ist in der Gesamtansicht der Trefferliste nicht möglich. Bitte zuerst mit Kommando a zu einem Eintrag springen." | ( if [ -z $wopt ]; then fmt -s -w $(tput cols); else tee; fi )
        else
            clear && printf '\e[3J'
            case $input in
        ...
        if [[ $filmnr -gt $trefferzahl || $filmnr -eq 0 ]]; then
            ...
            if [[ $input =~ ^d[hny]?[0-9]+$ && $noq -eq 0 ]]; then
                if [ "$filmurl" = "" ]; then
                    ...
                    echo -e "\033[1mFalls gewünscht, Speicherort und Dateiname anpassen.\033[0m"
                    if [[ "$ext" = m3u8* ]]; then
                        xx="mp4"
                    else
                        xx=$ext
...
bmcomm
filmmax=$(wc -l < $dir/bookmarks)   #Anzahl der Bookmarks
...
#### Bei fehlendem Suchbegriff wird die interne Kommando(Such-)zeile ohne Trefferliste geöffnet (Ausnahme: Option -l, Livestreams) bzw. bei bestimmten Optionen eine Meldung ausgegeben
if [[ -z $1 && -z $lopt ]]; then
    if ! [[ -z $nopt && -z $Aopt && -z $Bopt && -z $gopt && -z $kopt && -z $Kopt && -z $Lopt && -z $sopt && -z $topt ]]; then
...
if [ ! -z $lopt ]; then
    ...
    # Variable, um Suche in Film-URLs auszuschließen
    nourls=$(printf '.*'; printf '\\",\\".*%.0s' $(seq 1 12))
    ...
    # Suchergebnis für ersten Suchbegriff
    firstarg=$1
    [[ $firstarg = _* ]] && firstarg="${firstarg:1}"
    if [[ $firstarg = \+* ]]; then
        out=$(tail -n +2 $dir/filmliste | sed -n "/\b${firstarg:1}\b$nourls/$C p")   #Exakte Wortsuche
    else
        out=$(tail -n +2 $dir/filmliste | sed -n "/$firstarg$nourls/$C p")
    fi
fi

# Filtern mit weiteren Suchbegriffen
for i in "${@:2}"; do
    if [[ $i != _* ]]; then
        case $i in
            #Exakte Wortsuche
            \+*) out=$(echo "$out" | sed -n "/\b${i:1}\b$nourls/$C p") ;;
            #Ausschluss eines exakten Wortes
            \~\+*) out=$(echo "$out" | sed -n "/\b${i:2}\b$nourls/$C !p") ;;
            #Ausschluss eines Strings
            \~*) out=$(echo "$out" | sed -n "/${i:1}$nourls/$C !p") ;;
            #Normale Stringsuche
            *) out=$(echo "$out" | sed -n "/$i$nourls/$C p") ;;
        esac
    fi
done
...
for i in "${@:1}"; do
    if [[ $i = _* ]]; then
        s=${i:1}
        case $s in
            \+*)   out=$(echo "$out" | sed -n "/.*\",\"\[\"\",\"[^\"]*\b${s:1}/$C p") ;;
            \~\+*) out=$(echo "$out" | sed -n "/.*\",\"\[\"\",\"[^\"]*\b${s:2}/$C !p") ;;
            \~*)   out=$(echo "$out" | sed -n "/.*\",\"\[\"\",\"[^\"]*${s:1}/$C !p") ;;
            *)     out=$(echo "$out" | sed -n "/.*\",\"\[\"\",\"[^\"]*$s/$C p") ;;
        esac
    fi
done
...
if ! [[ "$datum1" = "01.01.0000" && "$datum2" = "31.12.9999" ]]; then
    out=$(echo "$out" | awk -F"\",\"" 'OFS="\",\""{ n=split($5,b,".");$5=b[3]"."b[2]"."b[1];print }' |   #Datumsfelder ($5) werden zwecks Vergleich invertiert \
        awk -F "\",\"" -v t1="$datum1" -v t2="$datum2" '{if (($5 <= t2)&&($5 >= t1)) {print} }' |   #Vergleich der Datumsfelder mit Optionsargumenten \
...
# Entfernen von Duplikaten (Option -e)
[ ! -z $eopt ] && out=$(echo "$out" | awk -F"\",\"" '!a[$10,$14,$16]++')
