...
#################################################################
#
#  Copyright (c) 2017-2022 Martin O'Connor (maroc@mailbox.org)
#
...
while getopts "A:B:bghHK:lL:noqQstuvw" opt; do
    case $opt in
        ...
        v)  if [ ! -f $dir/filmliste ]; then
            ...
            echo "MediaTerm 12.0, 2022-04-01"
...
while [ 1 ]; do
    ...
    elif [[ $input =~ ^d[hny]?[0-9]+$ || $input =~ ^[bhinwy][0-9]+$ || $input =~ ^[0-9]+$ ]]; then
        ...
        if [[ $filmnr -gt $trefferzahl || $filmnr -eq 0 ]]; then
            ...
            if [[ $input =~ ^d?y[0-9]+$ ]]; then
                ...
                if [ $? -eq 0 ]; then
                    ...
                    yt-dlp --no-colors --compat-options youtube-dl -F $weburl 2>/dev/null | grep -E '[0-9]k, ' | awk 'BEGIN{i=1} !/^\[/ && !/resolution/{print i++")\t"$0}' | grep . || warn=1
                    if [ $warn -eq 0 ]; then
                        read -p "Bitte Nummer der gewünschten Videoqualität eingeben (0 für \"Abbrechen\"): " nr
                        if [ $nr -gt 0 ]; then
                            yt=$(yt-dlp --no-colors --compat-options youtube-dl -F $weburl 2>/dev/null | grep -E '[0-9]k, ' | awk '!/^\[/ && !/resolution/{print $0}' | awk -v var=$nr 'NR==var{print $1}')
                            noq=0
...
if [ ! -z $bopt ]; then
    [ ! -f $dir/bookmarks ] && echo "Die Datei $dir/bookmarks existiert nicht." && exit
    bmcli
fi
