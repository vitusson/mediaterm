...
#################################################################
#
#  Copyright (c) 2017-2020 Martin O'Connor (mar.oco@arcor.de)
#
...
#### FUNKTIONEN hits, icli, bmcomm, bmcli, urlqual, pageview, dots
        ...
        if [[ $filmnr -gt $trefferzahl || $filmnr -eq 0 ]]; then
            ...
            if [[ $input =~ ^d?[nh][0-9]+$ ]]; then
                if [[ "$input" =~ ^d?n[0-9]+$ ]]; then
                    qual="niedriger"
                    if [ ! -z $lopt ]; then
                        range="480x272 480x270 426x240 400x224 512x288 640x360"   #Auflösungen für Livestreams in geringer Qualität
                    else
            ...
            if [[ ! -z $lopt && ! $input =~ ^d?o[0-9]+$ ]]; then
                ...
                if [[ $items -gt 1 ]]; then
                    for res in ${range}; do
                        filmurl_l=$(wget -q -O - $filmurl | grep -A1 "$res" | grep -v "$res" | tail -1)
                        if [[ "$filmurl_l" != "" ]]; then
