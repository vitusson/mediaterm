...
#################################################################
#
#  Copyright (c) 2017-2021 Martin O'Connor (maroc@mailbox.org)
#
...
while getopts "A:B:bghHK:lL:noqQstuvw" opt; do
    case $opt in
        ...
        v)  if [ ! -f $dir/filmliste ]; then
            ...
            echo "MediaTerm 11.0, 2021-05-11"
...
${fett}VORAUSSETZUNGEN FUER DAS FUNKTIONIEREN DES SKRIPTS:${normal}
      ffmpeg, mpv, wget und xz bzw. xz-utils müssen installiert sein.
...
function hits {
    ...
    # Ausgabe der Ergebnisliste bei nichtleerer Treffermenge
        ...
        if [ ! -z $lopt ]; then
            echo "$out" | \
                awk -F "\",\"" '{printf "("NR") " "\033[0;32m"$4"\033[0m"" ("$1")  [n/"} {if($4 == "3Sat Livestream" || $4 == "DW Livestream" || $4 == "SR Livestream" || $4 == "WDR Livestream"){print "-]"} else{print "h]"}}; {print "\033[0;34m"$10"\033[0m","\n"}' | \
...
while [ 1 ]; do
    ...
    elif [[ $input =~ ^(a|A)[0-9]?+$ ]]; then   #Anzeige von 5 bzw. 10 Treffern ab Zeile ...
        case $input in
            a*) pag=5 ;;
            A*) pag=10 ;;
        esac
        if [ $trefferzahl -lt $pag ]; then echo "Kommando wird bei weniger als $pag Treffern nicht unterstützt"
        else
            clear && printf '\e[3J'
            a=$(echo ${input//[!0-9]/})
            case $input in
                a*) let a=$(( a < trefferzahl ? a : trefferzahl-4 )) ;;
                A*) let a=$(( a < trefferzahl ? a : trefferzahl-9 )) ;;
            esac
            let a=$(( a > 0 ? a : 1 ))
            let x1=(a-1)*$tz+1
            case $input in
        ...
        if [[ $filmnr -gt $trefferzahl || $filmnr -eq 0 ]]; then
            ...
            if [[ $input =~ ^d?[nh][0-9]+$ ]]; then
                ...
                if [ ! -z $lopt ]; then
                    case $input in
                        dn*|n*)
                            bitrate="--hls-bitrate=2000000"
                            range="640x360 512x288 480x272 480x270 426x240 400x224" ;;
                        dh*|h*)
                            bitrate="--hls-bitrate=max"
                            range="1280x720 1024x576" ;;
                    esac
                else
                    case $input in
                       dn*|n*) def=14 ;;
            ...
            if [[ ! -z $lopt && ! $input =~ ^(i|w|b)[0-9]+$ ]]; then
                aud=0
                if wget -q -O - $filmurl | grep "TYPE=AUDIO" > /dev/null;then
                    aud=1
                fi
                if [[ "$MT_PLAYER" = mpv* && $aud -eq 1 ]];then
                    MT_PLAYER="$MT_PLAYER $bitrate"
                else
                    items=$(wget -q -O - $filmurl | grep -o RESOLUTION | wc -l)   #Anzahl der Video-Auflösungen
                    if [[ $items -gt 1 && $aud -eq 0 ]]; then
                        for res in ${range}; do
                            filmurl_l=$(wget -q -O - $filmurl | grep -A1 "^#EXT.*RESOLUTION=${res}" | grep -v "^#EXT.*RESOLUTION=${res}" | head -1)
                            [[ "$filmurl_l" = "" ]] || break
                        done
                ...
                if [ "$filmurl" = "" ]; then
                    ...
                    if [ ! -z $lopt ]; then
                        if [[ $aud -eq 1 ]]; then
                            ...
                            [[ "$MT_PLAYER" = mpv* ]] || echo "Es wird ein anderer Player als mpv genutzt. Damit wird bei diesem Livestream evtl. die Wahl der Filmqualität nicht unterstützt."
                        fi
                    fi
