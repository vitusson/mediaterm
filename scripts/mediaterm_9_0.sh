...
#----- Ende Textblock ------
    ...
    elif [[ $input =~ ^o[0-9]+$ && -z $lopt ]]; then
        echo "Das Kommando \"o\" kann nur auf Livestreams (Option -l) angewandt werden!"

    elif [[ $input =~ ^d[hn]?[0-9]+$ || $input =~ ^[bhinow][0-9]+$ || $input =~ ^[0-9]+$ ]]; then
        ...
        if [[ $filmnr -gt $trefferzahl || $filmnr -eq 0 ]]; then
            ...
            if [[ $input =~ ^d?[nh][0-9]+$ ]]; then
                if [[ "$input" =~ ^d?n[0-9]+$ ]]; then
                    qual="niedriger"
                    if [ ! -z $lopt ]; then
                        range="640x360 512x288"   #Auflösungen für Livestreams in geringer Qualität
                    else
...
if [[ ! -f $dir/filmliste  || ! -z $uopt ]]; then
    ...
    if [[ $antwort = J || $antwort = j || -z $antwort ]]; then
        ...
        if [ $? -ne 0 ]; then
            ...
            awk -F "\"" '!/\[""/ { sender = $2; } { print sender"\",\""$0; }' Filmliste-akt | awk -F "\",\"" -v OFS="\",\"" '!($3==""){ thema = $3; } {sub($3,thema,$3); print}' 2>/dev/null > filmliste   #allen Zeilen Sender voranstellen und in alle Zeilen Thema einfügen
            rm Filmliste-akt
